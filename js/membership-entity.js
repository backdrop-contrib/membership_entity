...
(function($) {
   ...
   */
  Backdrop.behaviors.fieldMembershipEntityJoin = {
   ...
   * account.
   */
  Backdrop.behaviors.MembershipEntityAddMembership = {
    attach: function (context, settings) {
      ...
      if ($checkbox.length) {
        $($checkbox, context).once('membership-entity-add-checkbox', function () {
          $(this).bind('change', function (e) {
            if ($(this).attr('checked')) {
              $submit.attr('value', Backdrop.t('Next: Membership Details'));
            }
            else {
              $submit.attr('value', Backdrop.t('Create new account'));
            }
          });
        });
      }
